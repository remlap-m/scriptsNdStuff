...
| summarize Count = count(), Devices = dcount(DeviceId) by ActionType, SensitivityLabel, RequestProtocol
| order by Count desc



// Pick a test file with a known label, copy it via robocopy or xcopy to another path/device
// on a test machine, then:
DeviceFileEvents
| where TimeGenerated > ago(1h)
| where FileName == "<your test filename>"
| project Timestamp, ActionType, DeviceName, FolderPath, PreviousFolderPath, SensitivityLabel, InitiatingProcessFileName, RequestProtocol, RequestAccountName, RequestSourceIP
